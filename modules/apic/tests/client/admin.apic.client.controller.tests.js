(function () {
  ...
  describe('Apic Admin Controller Tests', function () {
    // Initialize global variables
    var ApicAdminController,
      $scope,
      $httpBackend,
      $state,
      Authentication,
    ...
    beforeEach(inject(function ($controller, $rootScope, _$state_, _$httpBackend_, _Authentication_, _ApicService_, _Notification_) {
    ...
    describe('vm.save() as create', function () {
      ...
      it('should send a POST request with the form input values and then locate to new object URL', inject(function (ApicService) {
        // Set POST response
        $httpBackend.expectPOST('/api/apic', sampleApicPostData).respond(mockApic);
      ...
      it('should call Notification.error if error', function () {
        ...
        $httpBackend.expectPOST('/api/apic', sampleApicPostData).respond(400, {
        ...
        expect(Notification.error).toHaveBeenCalledWith({ message: errorMessage, title: '<i class="glyphicon glyphicon-remove"></i> Apic save error!' });
      });
    });
    ...
    describe('vm.save() as update', function () {
      ...
      it('should  call Notification.error if error', inject(function (ApicService) {
        var errorMessage = 'error';
        $httpBackend.expectPUT(/api\/apic\/([0-9a-fA-F]{24})$/).respond(400, {
          message: errorMessage
        });
    ...
    describe('vm.remove()', function () {
      ...
      it('should delete the apic and redirect to apic', function () {
        ...
        $httpBackend.expectDELETE(/api\/apic\/([0-9a-fA-F]{24})$/).respond(204);

        $scope.vm.remove();
        $httpBackend.flush();

        expect(Notification.success).toHaveBeenCalledWith({ message: '<i class="glyphicon glyphicon-ok"></i> Apic deleted successfully!' });
        expect($state.go).toHaveBeenCalledWith('admin.apic.list');
      });
